...
public class FieldOfView : MonoBehaviour
{
    ...
    public event Action<Soldier> OnSoldierFound;
    ...
    private void LateUpdate()
    {
       // IsTarget = false;
        ...
        for (int i = 0; i <= rayCount; i++)
        {
            ...
            //if (hit.collider != null && CompareLayer(hit.collider.gameObject.layer, soldierMask))
            //    IsTarget = true;

            if (hit.collider != null) {
                if (hit.collider.TryGetComponent(out Soldier _soldier)) {
                    //We have found the soldier
                    SoldierFound(_soldier);
                }
            }
    ...
    private void SoldierFound(Soldier _soldier) {
        //IsTarget = true;
        OnSoldierFound?.Invoke(_soldier);
    }
